...
for i = 1 : num_images
    ...
    train = csvread(fullfile(input_dir, filenames(i).name),1,1,[1,1,train_samp,8]);
    train = train.';
    training = horzcat(training, train);
    test = csvread(fullfile(input_dir, filenames(i).name),train_samp + 1 ,1,[train_samp + 1,1, train_samp  + test_samp,8]);
    test = test.';
